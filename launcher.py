...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Blue - Discord Bot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Blue",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Blue in case of issues",
                        action="store_true")
...
def reset_blue(reqs=False, data=False, cogs=False, git_reset=False):
    if reqs:
        try:
    ...
    if git_reset:
        ...
        if code == 0:
            print("Blue has been restored to the last local commit.")
        else:
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Blue (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            if user_pick_yes_no():
                reset_blue(data=True)
                wait()
        elif choice == "3":
            reset_blue(reqs=True)
            wait()
        elif choice == "4":
            print("Are you sure? This will wipe ALL your Blue's installation "
                  "data.\nYou'll lose all your settings, cogs and any "
                  "modification you have made.\nThere is no going back.")
            if user_pick_yes_no():
                reset_blue(reqs=True, data=True, cogs=True, git_reset=True)
                wait()
        elif choice == "0":
            break
        clear_screen()
...
def run_blue(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Blue. "
    ...
    print("Blue has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Blue without going
    ...
    start_blue = "{} --start".format(call)
    start_blue_autorestart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    start_blue             = ccd + start_blue             + pause
    start_blue_autorestart = ccd + start_blue_autorestart + pause

    files = {
        "start_blue"             + ext : start_blue,
        "start_blue_autorestart" + ext : start_blue_autorestart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Blue Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        print("4. Maintenance (repair, reset...)")
        print("\n0. Quit")
        choice = user_choice()
        if choice == "1":
            run_blue(autorestart=True)
        elif choice == "2":
            run_blue(autorestart=False)
        elif choice == "3":
            requirements_menu()
        elif choice == "4":
            maintenance_menu()
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Blue needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Blue cannot work without the pip module. Please make sure to "
    ...
    if INTERACTIVE_MODE:
        ...
        print("Starting Blue...")
        run_blue(autorestart=args.auto_restart)
