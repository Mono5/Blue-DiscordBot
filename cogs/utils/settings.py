...
class Settings:
    ...
    def parse_cmd_arguments(self):
        parser = argparse.ArgumentParser(description="Blue - Discord Bot")
        ...
        parser.add_argument("--co-owner", action="append", default=[],
                            help="ID of a co-owner. Only people who have "
                                 "access to the system that is hosting Blue "
        ...
        parser.add_argument("--admin-role", help="Role seen as admin role by "
                                                 "Blue")
        parser.add_argument("--mod-role", help="Role seen as mod role by Blue")
        ...
        parser.add_argument("--no-cogs",
                            action="store_true",
                            help="Starts Blue with no cogs loaded, only core")
        parser.add_argument("--self-bot",
                            action='store_true',
                            help="Specifies if Blue should log in as selfbot")
    ...
    @property
    def token(self):
        return os.environ.get("BLUE_TOKEN", self.bot_settings["TOKEN"])
    ...
    @property
    def email(self):
        return os.environ.get("BLUE_EMAIL", self.bot_settings["EMAIL"])
    ...
    @property
    def password(self):
        return os.environ.get("BLUE_PASSWORD", self.bot_settings["PASSWORD"])
