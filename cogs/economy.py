...
class Economy:
    ...
    def __init__(self, bot):
        ...
        if "PAYDAY_TIME" in self.settings:
    ...
    async def _set(self, ctx, user: discord.Member, credits: SetParser):
        ...
        Examples:
            bank set @Mono5 26 - Sets 26 credits
            bank set @Mono5 +2 - Adds 2 credits
            bank set @Mono5 -6 - Removes 6 credits"""
        author = ctx.message.author
        try:
    ...
    async def _server_leaderboard(self, ctx, top: int=10):
        ...
        bank_sorted = [a for a in bank_sorted if a.member]
