...
class Mod:
    ...
    def __init__(self, bot):
        ...
        self.ar_settings_path = "data/mod/ar_settings.json"
        self.ar_settings = dataIO.load_json(self.ar_settings_path)
        self.ar_users = {}
        self.ar_messages = {}

    def _ar_get_server_from_id(self, serverid):
        return discord.utils.get(self.bot.servers, id=serverid)

    def _ar_set_default(self, server):
        self.ar_settings[server.id] = {
            "ENABLED": False,
            "ROLE": None,
            "AGREE_CHANNEL": None,
            "AGREE_MSG": None
        }
        dataIO.save_json(self.ar_settings_path, self.ar_settings)

    async def _ar_no_perms(self, server):
    ...
    async def ar_on_message(self, message):
        server = message.server
        user = message.author
        if server is None:
            return
        if server.id not in self.ar_settings:
            self._ar_set_default(server)
            return
        try:
            if self.ar_settings[server.id]["AGREE_CHANNEL"] is not None:
                pass
            else:
                return
        except:
        ...
        try:
            if message.content == self.ar_users[user.id]:
                roleid = self.ar_settings[server.id]["ROLE"]
                try:
                    roles = server.roles
                ...
                try:
                    ...
                    if user.id in self.ar_messages:
                        self.ar_messages.pop(user.id, None)
                except discord.Forbidden:
                    if server.id in self.ar_settings:
                        await self._ar_no_perms(server)
        except KeyError:
            return

    async def _ar_agree_maker(self, member):
        server = member.server
        self.last_server = server
        await self._ar_verify_json(None)
        key = ''.join(random.choice(string.ascii_uppercase +
                                    string.digits) for _ in range(6))
        self.ar_users[member.id] = key
        ch = discord.utils.get(
            self.bot.get_all_channels(),
            id=self.ar_settings[server.id]["AGREE_CHANNEL"])
        msg = self.ar_settings[server.id]["AGREE_MSG"]
        try:
        ...
        try:
            ...
            return
        self.ar_messages[member.id] = msg

    async def _ar_auto_give(self, member):
        server = member.server
        try:
            roleid = self.ar_settings[server.id]["ROLE"]
            roles = server.roles
    ...
    async def _ar_verify_json(self, e, *a, **k):
        s = self.last_server
        if len(self.ar_settings[s.id].keys()) >= 4:
            return
        try:
            _d = self.ar_settings[s.id]
        except KeyError:
            self._ar_set_default(s)
        _k = _d.keys()
        ...
        if "AGREE_CHANNEL" not in _k:
            self.ar_settings[s.id]["AGREE_CHANNEL"] = None
        if "AGREE_MSG" not in _k:
            self.ar_settings[s.id]["AGREE_MSG"] = None

    async def _ar_roler(self, member):
        server = member.server
        self.last_server = server
        if server.id not in self.ar_settings:
            self._ar_set_default(server)

        if self.ar_settings[server.id]["ENABLED"] is True:
            try:
                if self.ar_settings[server.id]["AGREE_CHANNEL"] is not None:
                    await self._ar_agree_maker(member)
                else:
                    await self._ar_auto_give(member)
            except KeyError as e:
                self.last_server = server
                await self._ar_verify_json(e)
    ...
    async def autorole(self, ctx):
        ...
        if server.id not in self.ar_settings:
            ...
            dataIO.save_json(self.ar_settings_path, self.ar_settings)
        if "AGREE_MSG" not in self.ar_settings[server.id].keys():
            self.ar_settings[server.id]["AGREE_MSG"] = None
            dataIO.save_json(self.ar_settings_path, self.ar_settings)
        ...
        if ctx.invoked_subcommand is None:
            ...
            try:
                await self.bot.say("```Current autorole state: {}```".format(
                    self.ar_settings[server.id]["ENABLED"]))
            except KeyError:
                self._ar_set_default(server)
    ...
    async def role(self, ctx, role: discord.Role):
        ...
        server = ctx.message.server
        self.ar_settings[server.id]["ROLE"] = role.id
    ...
    async def agreement(self, ctx, channel: str,
        ...
        if channel == "clear":
            self.ar_settings[server.id]["AGREE_CHANNEL"] = None
            ...
            self.ar_settings[server.id]["AGREE_MSG"] = msg
    ...
    async def colour(self, ctx, role: discord.Role, value: discord.Colour):
        ...
        Examples:
        !editrole colour \"The Moderator\" #ff0000
...
def setup(bot):
    ...
    bot.add_listener(n._ar_roler, "on_member_join")
    bot.add_listener(n._ar_verify_json, "on_error")
    bot.add_cog(n)
