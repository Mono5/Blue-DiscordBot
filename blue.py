...
try:
    ...
    print("Discord.py not installed. Exitting...\n")
    sys.exit(1)
...
description = "Blue - Mono5's Developement Bot"
...
class Bot(commands.Bot):
    ...
    async def shutdown(self, *, restart=False):
        """Gracefully quits Blue with exit code 0

        If restart is True, the exit code will be 26 instead
        The launcher automatically restarts Blue when that happens"""
...
def set_logger(bot):
    ...
    blue_format = logging.Formatter(
        '%(asctime)s %(levelname)s %(module)s %(funcName)s %(lineno)d: '
        '%(message)s',
        datefmt="[%d/%m/%Y %H:%M]")

    stdout_handler = logging.StreamHandler(sys.stdout)
    stdout_handler.setFormatter(blue_format)
    ...
    fhandler.setFormatter(blue_format)
...
def load_cogs(bot):
    ...
    if owner_cog is None:
        print("The owner cog is missing. It contains core functions without "
              "which Blue cannot function. Reinstall.")
        exit(1)
